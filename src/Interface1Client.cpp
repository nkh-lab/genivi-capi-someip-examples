...
void Interface1Client::main() {
    ...
    myProxy->getAStringAttribute().getChangedEvent().subscribe(
            [this](std::string s) {
                LOG_INF << mClientInfo << ": aString was changed to: " << s;
            },
    ...
    while (true) {
        ...
        if (CHECK(callStatus == CommonAPI::CallStatus::SUCCESS,
                  "%s: getAStringAttribute().setValue() call failed!", mClientInfo.c_str())) {
        ...
        CHECK(callStatus == CommonAPI::CallStatus::SUCCESS, "%s: setUInt32() call failed!", mClientInfo.c_str());

        std::this_thread::sleep_for(std::chrono::seconds(3));

        myProxy->getUInt32(callStatus, g);
        CHECK(callStatus == CommonAPI::CallStatus::SUCCESS, "%s: getUInt32() call failed!", mClientInfo.c_str());
